...
namespace NetworkHelper
{
    ...
    struct io_context_t
    {
        ...
        io_context_t() : socket(INVALID_SOCKET),
    ...
    ClientHelper::ClientHelper() : hIOCP(INVALID_HANDLE_VALUE),
                                   hExitFlag(NULL),
                                   hGlobalEvent(NULL),
                                   threads(0),
                                   workers(NULL),
    ...
    inline void ClientHelper::init()
    {
        ...
        if (result != 0)
        {
            quitWithError("WSAStartup failed");
        }
        ...
        if (h != hIOCP)
        {
            quitWithError("create Iocp failed");
        }
