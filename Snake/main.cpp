#include <winsock2.h>
#include "MenuScene.h"
#include "GameManager.h"
#include "Common.h"
#include <iostream>
#include <windows.h>
#pragma comment(linker, "/subsystem:\"windows\" /entry:\"mainCRTStartup\"")

// ============== 主函数 ==============
int main()
{
    // 启用高DPI感知，防止Windows缩放导致画面模糊
    SetProcessDPIAware();

    // 隐藏控制台窗口
    HWND hwnd = GetConsoleWindow();
    if (hwnd != NULL)
    {
        ShowWindow(hwnd, SW_HIDE);
    }

    // 初始化随机数种子
    Utils::InitRandom();

    // 创建全局窗口（最大尺寸，足够容纳菜单和游戏）
    int maxWidth = 1920;  // 1080p宽度
    int maxHeight = 1080; // 1080p高度
    initgraph(maxWidth, maxHeight);
    SetWindowText(GetHWnd(), L"贪吃蛇游戏");

    // 禁用最大化按钮
    HWND gameHwnd = GetHWnd();
    SetWindowLong(gameHwnd, GWL_STYLE, GetWindowLong(gameHwnd, GWL_STYLE) & ~WS_MAXIMIZEBOX);

    // 主循环：菜单 -> 游戏 -> 菜单
    while (true)
    {
        // 显示菜单（不管理窗口）
        MenuScene menu(false);
        GameMode selectedMode = menu.Show();

        // 用户选择退出
        if (selectedMode == EXIT)
        {
            break;
        }

        // 创建游戏管理器
        GameManager game;

        // 初始化游戏
        game.Init(selectedMode);

        // 运行游戏主循环
        game.Run();

        // 游戏结束，清空屏幕准备下一次循环
        cleardevice();
    }

    // 关闭全局窗口
    closegraph();

    return 0;
}

/*
 * ============== 项目结构说明 ==============
 *
 * 📂 基础公共模块 (Infrastructure)
 *    - Common.h          : 全局常量、枚举、结构体定义
 *    - Utils.h           : 工具函数集合
 *
 * 📂 接口层 (Interfaces)
 *    - IController.h     : 控制器接口（解耦核心）
 *
 * 📂 游戏实体模块 (Game Objects / Model)
 *    - Snake.h/.cpp      : 蛇的本体
 *    - GameMap.h/.cpp    : 地图数据管理
 *    - FoodManager.h/.cpp: 食物生成与管理
 *
 * 📂 控制策略模块 (Strategies / Controller)
 *    - KeyboardController.h/.cpp : 本地玩家控制器
 *    - NetworkController.h/.cpp  : 网络（对手）控制器
 *    - AIController.h/.cpp       : 电脑控制器（加分项）
 *
 * 📂 渲染模块 (View)
 *    - Renderer.h/.cpp   : 封装EasyX操作
 *
 * 📂 网络通信模块 (Network)
 *    - Client.h          : 助教提供的网络框架
 *    - NetworkManager.h/.cpp : 游戏逻辑与底层网络的适配器
 *
 * 📂 系统核心模块 (System)
 *    - GameManager.h/.cpp: 核心状态机
 *    - MenuScene.h/.cpp  : 菜单场景
 *    - main.cpp          : 程序入口
 *
 * ============== 开发建议 ==============
 *
 * Step 1: 确保编译通过
 *    - 添加所有.cpp文件到项目
 *    - 配置EasyX库
 *    - 编译检查是否有语法错误
 *
 * Step 2: 实现渲染模块
 *    - 在 Renderer.cpp 中实现基本的绘图功能
 *    - 测试能否显示一个静态的蛇
 *
 * Step 3: 实现蛇的移动
 *    - 在 Snake.cpp 中实现 Move() 和 Update()
 *    - 在 KeyboardController.cpp 中实现键盘检测
 *    - 测试能否用键盘控制蛇移动
 *
 * Step 4: 完善单机游戏
 *    - 实现 FoodManager
 *    - 实现 GameMap 的碰撞检测
 *    - 实现 GameManager 的主循环
 *    - 此时应该能玩完整的单机版
 *
 * Step 5: 添加网络功能
 *    - 集成助教提供的 Client.h
 *    - 实现 NetworkManager
 *    - 实现 NetworkController
 *    - 测试网络对战
 *
 * Step 6: 添加高级功能
 *    - 实现 AI 控制器
 *    - 实现进阶版/高级版的特殊规则
 *    - 实现记录系统
 *
 * ============== 注意事项 ==============
 *
 * 1. 所有TODO标记的地方都需要你实现
 * 2. 保持接口不变，只实现函数体
 * 3. 测试时逐步添加功能，不要一次全写完
 * 4. 网络部分可以最后实现，先确保单机版能玩
 * 5. 记得处理内存释放，避免内存泄漏
 *
 */
